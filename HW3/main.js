...
//1.2
const getPizzasNamesNext = () => {
  let array = [];
  for (let pizza of pizzaList) {
    array.push(pizza.name);
  }
  console.log(array)
};
getPizzasNames()
...
getPizzaByIdSecond(2);

//2.3
const getPizzaByIdThird = id =>{
  pizzaList.forEach(item =>{
    if(item.id === id){
      console.log(item);
    }
...
//3
const getPizzasByComposition = (name) => {
  let array = [];
  for (let pizza of pizzaList) {
    let compositions = pizza.composition;
...
getPizzasByComposition('моцарелла')

//3.2
const getPizzasByCompositionNext = name =>{
  let array = [];
  pizzaList.forEach(pizza =>{
    let composition = pizza.composition;
...
getPizzasByCompositionNext('ветчина');
...
const getPizzasByCalloricity = caloricity => {
  let array = [];
  for (let pizza of pizzaList) {
    if (pizza.caloricity > maxCaloricity) {
      array.push(pizza)
    }
  }
  console.log(array);
...
const sortByMaxCompositions = compositions => {
  ...
  for(let i = 0; i<pizzaList.length; i++){
    let pizza = pizzaList[i];
    if(pizza.composition.length > maxCompositions){
      newArray.push(pizza)
    }
  }
  console.log(newArray)
